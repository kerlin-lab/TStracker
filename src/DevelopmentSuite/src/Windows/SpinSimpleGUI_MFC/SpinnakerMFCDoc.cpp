...
BOOL CSpinnakerMFCDoc::OnNewDocument()
{
    ...
    if (m_continueGrabThread)
    {
        ...
        if (dwRet == WAIT_TIMEOUT)
        {
            ...
            CString csMessage;
            csMessage.Format("Failed to stop current grab thread.");
    ...
    try
    {
        ...
        if (!m_devInfo.isCamera)
        {
            ...
            {
                CString csMessage;
                csMessage.Format("Connect Failure: Failed to get camera from camera selection dialog.");
                AfxMessageBox(csMessage, MB_ICONSTOP);
    ...
    {
        CString csMessage;
        csMessage.Format("Connect Failure: %s (Error %s)", "Failed to get camera from index 0", ex.what());
        AfxMessageBox(csMessage, MB_ICONSTOP);
...
UINT CSpinnakerMFCDoc::DoGrabLoop()
{
    ...
    {
        csMessage.Format("StartCapture Failure: %s", e.what());
        AfxMessageBox(csMessage, MB_ICONSTOP);
        return 0;
    }
    ...
    while (m_continueGrabThread)
    {
        try
        {
            ...
            {
                csMessage.Format("Convert Failure: %s", e.what());
                continue;
            }
        ...
        {
            csMessage.Format("Grab loop Failure: %s", e.what());
            AfxMessageBox(csMessage, MB_ICONSTOP);
            return 0;
        }
    }
    ...
    {
        csMessage.Format("Stop Failure: %s", e.what());
        AfxMessageBox(csMessage, MB_ICONSTOP);
        return 0;
    }
...
void CSpinnakerMFCDoc::OnFileSaveAs()
{
    ...
    const CString arcsFilter[uiNumFilters] = {"Windows Bitmap (*.bmp)|*.bmp",
    ...
    // Retrieve Device ID
    Spinnaker::GenApi::CStringPtr ptrStringSerial = m_pCamera->GetTLDeviceNodeMap().GetNode("DeviceID");

    char tempFilename[128];
    sprintf(tempFilename, "%s-%s", ptrStringSerial->GetValue().c_str(), timestamp);

    CFileDialog fileDialog(
        FALSE, "bmp", tempFilename, OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT, csFilters, AfxGetMainWnd());
    ...
    if (fileDialog.DoModal() == IDOK)
    {
        ...
        if (saveImageFormat == Spinnaker::RAW)
        {
            try
            {
                m_saveImage->Save(fileDialog.GetPathName(), Spinnaker::RAW);
            }
            catch (Exception& ex)
            {
                csMessage.Format("Failed to save image (Error: %s)", ex.what());
                AfxMessageBox(csMessage, MB_ICONSTOP);
            }
        }
        else if (saveImageFormat == Spinnaker::PGM)
        {
                ...
                tempPixelFormat == PixelFormat_Raw16)
            {
                try
                {
                    m_saveImage->Save(fileDialog.GetPathName(), saveImageFormat);
                }
                catch (Exception& ex)
                {
                    csMessage.Format("Failed to convert image (Error: %s)", ex.what());
                    AfxMessageBox(csMessage, MB_ICONSTOP);
                }
            }
            else
        ...
        else
        {
            ...
            try
            {
                convertedImage->Save(fileDialog.GetPathName(), saveImageFormat);
            }
            catch (Exception& ex)
            {
                csMessage.Format("Failed to save image (Error: %s)", ex.what());
